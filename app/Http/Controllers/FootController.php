...
class FootController extends Controller
{
    ...
    public function storeMovement(Request $request)
    {
        ...
        if ($request->movementType === 'Registrar producción') {
            ...
            foreach ($data['pedicureExpert']['products'] as &$product) {
            ...
            $data['pedicureExpert']['current_production'] += $totalKitsProduction;
        } elseif ($request->movementType === 'Entrada de producto' || $request->movementType === 'Salida de producto') {
            $productIndex = collect($data['pedicureExpert']['products'])->search(fn($prod) => $prod['name'] === $request->product);

            if ($productIndex === false) {
                return response()->json(['error' => 'Producto no encontrado'], 404);
            }

            $product = &$data['pedicureExpert']['products'][$productIndex];
    ...
    public function storeNewProduction(Request $request)
    {
        ...
        $data['pedicureExpert']['goal'] = intval($request->goal);
        $data['pedicureExpert']['current_production'] = intval($request->current_production);
