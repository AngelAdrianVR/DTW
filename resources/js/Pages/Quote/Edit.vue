<template>
    <AppLayout title="Cotizacion-editar">
        <template #header>
        ...
        <div class="lg:w-11/12 lg:grid mx-auto grid-cols-2 gap-5 mt-6">
            <form @submit.prevent="update" class="shadow-md py-4 mb-6 px-5 bg-white rounded-lg">
                <div class="mt-3">
                ...
                <div class="mt-3">
                    <div class="flex space-x-2">
                        <el-input class="w-3/4" v-model="new_feature" type="text" placeholder="Características incluidas">
                            <template #prefix>
                                <el-tooltip content="Caracterísicas incluidas en la lcotización" placement="top"
                                    effect="dark">
                    ...
                    <el-select class="mt-2 w-full" v-model="form.included_features" multiple clearable
                        placeholder="Caracteristicas" no-data-text="Agrega primero una caracteristica">
                ...
                <div class="mt-3">
                    <div class="flex space-x-2">
                        <el-input class="w-2/3" v-model="form.offer_validity_days" type="number"
                            ...
                            <template #prefix>
                                <el-tooltip content="Dias de validez de cotizacion despues de emision" placement="top"
                                    effect="dark">
                                    <el-icon class="el-input__icon"><i class="fa-solid fa-stopwatch"></i></el-icon>
                                </el-tooltip>
                            </template>
                        </el-input>
                        <SecondaryButton class="w-1/3" :type="'button'" @click="setExpiredDate">Calcular fecha
                        </SecondaryButton>
                    </div>
            ...
            <div class="text-sm shadow-md py-2 mb-6 px-3 bg-white rounded-lg relative">
                ...
                <p class="text-xs text-right">Fecha de emisión: <span id="fecha-emision">{{ formatearFecha(fechaEmision)
                }}</span></p>
                ...
                <div class="px-4 mt-2">
                    ...
                    <ul class="ml-4 mt-2" v-if="form.included_features?.length">
                        <li v-if="form.subtitles?.length" class="text-sm font-bold text-[#7F659C] text-left mt-2">
                            {{ form.subtitles }}</li>
                        <li class="text-xs" v-for="(feature, index) in form.included_features" :key="feature"><span
                                class="mr-3">{{ (index + 1) + '. ' }}</span>{{ feature }}</li>
                    </ul>
                    <p v-if="form.total_work_days" class="text-sm font-bold text-left mt-2">Duración</p>
                    <p class="text-xs" v-if="form.total_work_days">La entrega estimada para la implementación final del
                        proyecto es {{ form.total_work_days }} días hábiles, iniciando a partir del primer pago al
                        inicio del proyecto.
                    </p>
                    <p v-if="form.total_cost" class="text-sm font-bold text-left mt-2">Costo</p>
                    <p class="text-xs" v-if="form.total_cost">${{
                    ...
                    <p v-if="form.percentage_discount" class="text-xs">Descuento: ${{ ((form.percentage_discount * 0.01) *
                        form.total_cost).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }} (%{{ form.percentage_discount
    }})</p>
                    <p v-if="form.percentage_discount" class="text-sm font-bold text-left mt-2">Total</p>
                    <p class="text-xs" v-if="form.percentage_discount">${{ (form.total_cost - (form.percentage_discount *
                        0.01) * form.total_cost).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }} IVA incluido</p>
                    ...
                    <p class="text-xs mt-1">Si se requiere adicional Dominio y Hosting, se puede solicitar con un costo
                        extra por la adquisición de los mismo. </p>
                    <p class="text-xs font-bold mt-3">Proceso</p>
                    <p class="text-xs">El proyecto parte desde cero, comenzando con la fase de diseño. En esta etapa, se
                        envía al cliente un archivo que contiene
                        todas las vistas planificadas para la aplicación. Una vez que el diseño es aprobado, avanzamos hacia
                        el desarrollo de la
                        programación.
                        ...
                        Adicionalmente, ofrecemos una capacitación en línea para un máximo de 5 usuarios seleccionados por
                        la empresa. Esto
                        garantiza que el cliente pueda aprovechar al máximo la nueva aplicación.
                        Para respaldar nuestro compromiso con la calidad, proporcionamos un año de soporte técnico integral
                        para solucionar
                    ...
                    <p class="text-xs">- Compatibilidad en todos los dispositivos: nuestra plataforma es compatible con una
                        amplia gama de dispositivos
                        (computadora de escritorio, laptop, Tablet y/o teléfono móvil)
                    </p>
                    <p class="text-xs">- Seguridad de datos: la información se almacena de manera segura en la nube para
                        proteger los datos de la
                        empresa contra pérdidas. Realizamos respaldos automáticos para garantizar que sus datos estén
                        siempre
                        protegidos.
                    </p>
                    <p class="text-xs">- Acceso remoto: Los usuarios pueden acceder a la información de la empresa desde
                    ...
                    <p class="text-xs">- Soporte: Ofrecemos soporte técnico para garantizar que los usuarios aprovechen al
                        máximo la plataforma y
...
export default {
    data() {
        const form = useForm({
            quote_name: this.quote.quote_name,
            customer_name: this.quote.customer_name,
            company: this.quote.company,
            company_address: this.quote.company_address,
            quote_description: this.quote.quote_description,
            project: this.quote.project,
            subtitles: [],
            included_features: this.quote.included_features,
            suggested_features: [],
            promised_end_date: this.quote.promised_end_date,
            ...
            total_hours: this.quote.total_hours,
